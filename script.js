document.addEventListener('DOMContentLoaded', () => {
    ...
    const render = () => {
        // LERP factor determines how 'snappy' or 'floaty' the tracking is (0.1 is smooth)
    ...
    }, { threshold: 0.5 }); // Triggers when 50% of the SVG hits the viewport

    scribbles.forEach(svg => observer.observe(svg));
    ...
    document.querySelectorAll('.typewriter-text').forEach(el => {
        ...
        scrollTypewriters.push({ el, spans, top: 0 });
    });
    ...
    const initializeLayoutMaps = () => {
        ...
        // Build Parallax map
        parallaxTargetElements = Array.from(document.querySelectorAll('.project-card')).map(card => {
